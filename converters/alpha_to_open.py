...
def main():
    ...
    output_dir = os.path.join(os.path.dirname(os.path.dirname(alphapose_json_path)), 'openpose_keypoints_45')
    os.makedirs(output_dir, exist_ok=True)

    # Process each person's AlphaPose entry and save as OpenPose JSON
    for person_entry in alpha_data:
        openpose_data = convert_alpha_to_openpose(person_entry)
        file_id = person_entry["image_id"]
        file_name, ext = os.path.splitext(file_id)
        output_filename = os.path.join(output_dir, f'{file_name}_openpose_keypoints.json')
